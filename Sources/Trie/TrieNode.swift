...
struct TrieNode<Value> {
    ...
    typealias TrieNodes = [Character: TrieNode]
    ...
    func child(for character: Character) -> TrieNode? {
        children[character]
    }
    ...
    mutating func insert<Suffix>(_ suffix: Suffix, _ value: Value) where Suffix: StringProtocol {
        if let first = suffix.first {
            var child = child(for: first) ?? TrieNode(character: first)
...
extension TrieNode: Sendable where Value: Sendable {
}
