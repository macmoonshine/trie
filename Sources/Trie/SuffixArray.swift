...
/// A sorted array that functions like a suffix trie.
public struct SuffixArray<Value>: Trie {
    ...
    func index<Key>(key: Key, isSuffix: Bool = false) -> Array.Index? where Key : StringProtocol {
        let key = key.reversedString
        guard let index = array.bisectFirstIndex(where: { $0.key >= key }) else { return nil }
        let firstKey = array[index].key
        
        return key == firstKey || isSuffix && firstKey.hasPrefix(key) ? index : nil
    }
    
    func range<Key>(suffix: Key) -> Range<Array.Index>? where Key : StringProtocol {
        guard let index = index(key: suffix, isSuffix: true) else { return nil }
        let key = suffix.reversedString
    ...
    public func contains<Key>(any suffix: Key) -> Bool where Key : StringProtocol {
        index(key: suffix, isSuffix: true) != nil
    }
    ...
    public func values<Key>(for key: Key) -> Values where Key : StringProtocol {
        guard let range = range(suffix: key) else { return [] }
    ...
    public mutating func insert<Key>(_ key: Key, _ value: Value) where Key : StringProtocol {
        ...
        if let index = array.bisectFirstIndex(where: { $0.key >= key }) {
            if array[index].key == key {
                ...
                array.insert((key, value), at: index)
            }
        }
        else {
    ...
    public func dictionary() -> [String : Value] {
        array.reduce(into: [String: Value]()) {
            $0[$1.key.reversedString] = $1.value
        }
    }
}

extension SuffixArray: Sendable where Value: Sendable {
}

extension SuffixArray: Codable where Value: Codable {
...
extension SuffixArray: ExpressibleByDictionaryLiteral {
    public init(dictionaryLiteral elements: (String, Value)...) {
        array = elements.map({ ($0.0.reversedString, $0.1) })
            .sorted { $0.0 < $1.0 }
    }
}

extension SuffixArray: ExpressibleByArrayLiteral where Value == String {
    ...
    public init(_ strings: [String]) {
        array = strings.map({ ($0.reversedString, $0) })
            .sorted { $0.0 < $1.0 }
    }
