...
/// A prefix trie.
public struct PrefixTrie<Value> {
    public typealias Values = [Value]
    typealias Node = TrieNode<Value>
    typealias Nodes = [Character: Node]
        
    var root: Node = Node(character: "\0")
    ...
    public func contains<Key>(prefix: Key) -> Bool where Key: StringProtocol {
    ...
    public mutating func remove<Key>(prefix: Key) where Key: StringProtocol {
